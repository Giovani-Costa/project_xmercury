...
with cluster.connect() as session:
    ...
    VALUES (7b32de93-92b1-402a-93fe-c7a295535490, 'Forja', 'Sua maestia na forja é admirável. Você pode forjar ferramentas se tiver os materiais e equipamentos necessários, sendo eles: calor, material, martelo e uma base.', false, false, Null);"""
    )
    session.execute(
        """INSERT INTO xmercury.pericias (id, nome, descricao, e_vantagem, e_soma, somar)
    ...
    #     session.execute(
    #         """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
    # VALUES (7580cf09-eabd-4b2a-8798-7686467c9a9b, 'Corte com a Masked Death', 0, 'acao', 'ATAQUE, CORTANTE, VERSÁTIL', '8m.', 'Instatânea.', '1d20 + Bônus de FOR + 2.', '1d8 + 1d10 + Bônus de FOR', 'Caso você erre no teste de acerto, você solta a **ARMA** e terá que gastar sua Ação Bônus para pega-la novamente.', 'None', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    #     )
    #     session.execute(
    #         """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
    # VALUES (27c56bcd-d9b9-49f2-809c-5877ffdd3e2a, 'Masked Death: Fools Blood', 1, 'acao', 'ATAQUE, VERSÁTIL, NECRÓTICO, MÁGICO', '8m.', 'Instântanea.', '1d20 + Bônus de FOR + 2.', '1d8 + 1d10 + Atributo-chave de conjuração + 2 de dano **NECRÓTICO**.', 'O encantamento acaba', 'None', 'O usuário precisa ativar o encantamento da espada para usar essa habilidade, usando sangue ou algumas gotas de uma poção de cura.', 'None', 'Criaturas em um raio de 2m.', 'Igual ao bônus de proficiência do usuário.', 'None', 'None', 'None', 0, 'None');"""
    #     )
    #     session.execute(
    #         """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
    # VALUES (b3f4c9e0-7a6d-4e3b-bd92-5a1f6c8d0e5f, 'Ataque Poderoso', 'Você treinou seu corpo para ser capaz de desferir ataques poderosíssimos sempre que ataca. Seus **ATAQUES** com **ARMAS** recebem a seguinte modifi
    # ão:', 'reacao', 'AMPLIAR + 15', '**Acerto:** você causa +1d8 pontos de dano do mesmo tipo.', 0, 'PE');"""
    #     )
    #     session.execute(
    #         """
    # """
    ...
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)       
VALUES (e4d7675d-3cb7-4395-8de0-5c4c17a8cc83, 'Ataque Especializado', 'Você aprendeu a se virar em combate usando astúcia e conhecimento com suas armas. Seus **ATAQUES** com **vantagem** que causam dano recebem a seguinte modificação:', 'reacao', 'ADICIONA', '**Acerto:**  Você causa +1d6 pontos de dano do mesmo tipo.\n**Especial:** Você só pode causar esse dano adicional uma vez por turno.', 0, 'PE');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (bb85504b-ff0b-4e3f-ae2c-5e0effb87f2b, 'Disparo com a Head Hunter', 0, 'acao', 'DISPARÁVEL, PERFURANTE, PRECISO, ARMA, ESPALHAFATOSO', '18m.', 'Instantânea.', '1d20 + Bônus de DES + 2 vs DES.', '2d8 pontos de dano.', 'Caso você erre no teste de acerto, você solta a **ARMA** e terá que gastar sua Ação Bônus para pega-la novamente.', 'Você dispara com a sua **ARMA**.', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (8dea83d1-5beb-4c02-a29f-322205850046, 'Corte com a Masked Death', 0, 'acao', 'ATAQUE, CORTANTE, VERSÁTIL, ALONGADOS', '6m.', 'Instatânea.', '1d20 + Bônus de FOR + 2.', '1d8 + 1d10 + Bônus de FOR', 'Caso você erre no teste de acerto, você solta a **ARMA** e terá que gastar sua Ação Bônus para pega-la novamente.', 'None', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (b7864548-2ebe-4823-a884-d1a6455d6a7f, 'Masked Death: Fools Blood', 1, 'acao', 'ATAQUE, VERSÁTIL, NECRÓTICO, MÁGICO, ALONGADOS', '6m.', 'Instântanea.', '1d20 + Bônus de FOR + 2.', '1d8 + 1d10 + Atributo-chave de conjuração + 2 de dano **NECRÓTICO**.', 'O encantamento acaba', 'None', 'O usuário precisa ativar o encantamento da espada para usar essa habilidade, usando sangue ou algumas gotas de uma poção de cura.', 'None', 'Criaturas em um raio de 2m.', 'Igual ao bônus de proficiência do usuário.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.talentos (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (961e8d2d-c360-4751-9829-7786acf4e6aa, 'Se Dá Certo; Não é Estúpido; é Genial!', 'Diferente dos outros aventureiros, você é considerado proficiente com ataques improvisados, pode usar seus pontos de ênfase neles e causa o dobro dos dados de dano que causariam. Além disso, se a arma for um item pequeno (volume 1 ou menos), recebe o descritor ARMA e um descritor à sua escolha entre **ARREMESSÁVEL** ou **LEVE**; se for um item grande (volume 3 ou mais), precisa usá-lo com as duas mãos e ele recebe um descritor à sua escolha entre **ALONGADA** ou **ESPALHAFATOSA**. (Já adicionado na ficha)', 'None', 'None', '0', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (de8f9988-529d-401e-9987-409fab27288a, 'Resistir com Unhas e Dentes', 1, 'reacao', 'ESPECIALISTA', 'Pessoal.', 'Instantâneo.', 'None', 'None', 'None', 'Jogue seu dado de _Ataque Especializado_. Você usa o resultado da jogada para aumentar sua redução contra o dano sofrido.', 'Você só pode usar essa habilidade uma vez por rodada.', 'Você sofre dano.', 'O usuário.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (7bea74b6-bf1c-4d3d-807d-8498bc2987ba, 'Energia Negativa', 'Todos os seus **ATAQUES** com **ARMA** ou que causem dano **NECRÓTICO** causam +2 pontos de dano **NECRÓTICO**. Além disso, sempre que você ou uma de suas **CRIAÇÕES** causarem dano **NECRÓTICO** com um **ATAQUE**, o alvo fica ***Marcado*** ou aumenta essa condição em +1. Além disso, quando um reanimado sob seu controle for eliminado, a criatura que o eliminou recebe a condição ***Marcado***, ou aumenta essa condição em +1. Por fim, sempre que uma criatura ***Marcada*** em até 18m de você morrer, você pode transferir essa condição para você ou para um Reanimado sob seu controle.', 'None', 'None', '0', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (685388af-e4d2-4f4f-8b16-029910c08ba6, 'Necromancia Aprimorada', 2, 'acao bonus', 'NECROMANTE, MÁGICO', '9m.', 'None', 'None', 'None', 'None', 'O usuário pode consumir qualquer número de marcas de criaturas em até 18m de você para aprimorar qualquer número de reanimados que você controle. Você escolhe como distribuir as melhorias por marca. Veja a tabela “_Reanimado Aprimorado_” para mais informações.', 'None', 'None', 'Um reanimado controlado pelo usuário.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (2c09f75c-18ad-40af-bb7c-d3cc01132f42, 'Exército de Ossos', 'O usuário é capaz de invocar mortos-vivos chamados reanimados e depois comandá-los em combate. O reanimado passa a ser considerado uma **CRIAÇÃO** e não pode agir durante uma cena de combate. Sempre que você se deslocar, seus reanimados podem se deslocar uma distância igual à que você deslocou-se. Você pode usar as habilidades “_Reanimar Corpos_”, “_Comandar Corpos_” e “_Necromancia Aprimorada_”.', 'None', 'None', '0', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (d3c3086e-4324-4a3f-bf3b-135b8acded07, 'Reanimar Corpos', 1, 'acao bonus', 'NECROMANTE, MÁGICO, CRIAÇÃO', '6m.', 'Até ser destruído.', 'None', 'None', 'None', 'Você invoca uma criatura morta-viva chamada reanimado e ela recebe a condição ***Marcado***.', 'Se tiver a “_Tradição da Invocação_”, pode considerá-lo como sua ***Invocação Sintonizada***.', 'None', 'Um espaço desocupado no alcance.', 'Ilimitado.', 'reacao', 'ADICIONA', '**Efeito**: É possível gastar uma marca de uma criatura Marcada dentro do alcance para invocar um reanimado adicional dentro do alcance. Você pode controlar um máximo de reanimados igual ao seu ***bônus de proficiência***.', 1, 'PE');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (64e4a387-ac93-4146-957f-f38c3f6b6e10, 'Receber Benção', 2, 'acao livre', 'ESPECIALISTA', 'Pessoal.', 'Até o próximo descanso longo.', 'None', 'None', 'None', '⌂O usuário ativa todas suas benções.', 'O usuário só pode usar essa habilidade no final de uma Cena de Descanso e somente uma vez por cena.', 'None', 'O usuário.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (43d8755b-a5f4-4c4d-8aa5-ee3e235b80a1, 'Alimentos das Chamas', 2, 'acao', 'BENÇÃO, MÁGICO', 'Visão.', 'Instântanea.', 'Mágico vs **CAR**.', 'O usuário descobre o que aquela criatura mais deseja (naquela cena).', 'O usuário só pode usar essa habilidade se tiver usado a habilidade "_Receber Benção_"', 'Nenhuma outra personagem ou criatura sabe que você usou essa habilidade.', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (e314977f-8d03-4eba-9b24-9778b689331b, 'Engolfar em Chamas da Vingança', 1, 'acao bonus', 'BENÇÃO, MÁGICO, ÍGNEO', 'Pessoal.', 'Até o fim da próxima cena.', 'None', 'None', 'None', 'Todo o dano que você causar passa a ser **ÍGNEO**.', 'O usuário só pode usar essa habilidade se tiver usado a habilidade "_Receber Benção_', 'None', 'Você.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (d24b8e11-6d39-4228-8211-55f2458fb72c, 'Masked Death', 'Uma espada longa com o encantamento _"Fools Blood"_, a espada é bem trabalhada e possui a inscrição "Masked Death" em sua lâmina, que faz referência à banda de seu antigo portador, Zombie. O encantamento precisa de um pouco de sangue ou algumas gotas de poção de cura para ativar suas cargas, depois da ativação a espdada fica vermelha e pode usar a habilidade _"Masked Death: Fools Blood"_.', 500, 2);"""
    )
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (52056c6a-5200-4d16-8ecc-cde1ba3e5cd2, 'Head Hunter', 'Um grande rifre vermelho com detalhes pretos criado apatir da imaginação de seu portador. "Isso me lembra a sniper do Cypher" -- disse Chrollo durante o acampamento na Black Rose Forest', 750, 5);"""
    )
    session.execute(
        """INSERT INTO xmercury.personagens (id, nome, nickname, level, path, classe, legacy, heritage, melancholy, catarse, pe, hp, reducao_de_dano, bonus_de_proficiencia, pericias, talentos, passivas, skills, forca, dexterity, constituicao, inteligencia, sabedoria, carisma, pontos_de_sombra, resistencia, vulnerabilidade, imunidade, inventario_itens, inventario_numero, condicoes, saldo, imagem, usuario)
VALUES (69fa11c2-ca6a-44b7-93c2-b744d0e98554, 'Julius Wick', 'Julius', 4, 'Necromante','Especialista', 'Humano', 'Pomonas Cycle', 'Para Julius, o fim é necessário. Assim como a vida, todo ciclo tem um fim.', 1, 12, 36, 2, 2, [c25f6a18-6b01-423a-9c44-1781f677137d, 1030947f-9e65-476a-b08c-c834a2ddfe7f, a052505a-add0-4717-9d30-e382a0741058, 3bc86566-ec94-4459-a7fc-2a5d094a1f39, d28b4723-1177-46ac-b1f2-bf785330b1a9, c57e4a9b-2e9d-4c97-9f6c-0139cd0ddb44], [94c51d1e-c688-4185-9c62-3f1782fd4651, 961e8d2d-c360-4751-9829-7786acf4e6aa], [7bea74b6-bf1c-4d3d-807d-8498bc2987ba, 2c09f75c-18ad-40af-bb7c-d3cc01132f42], [bb85504b-ff0b-4e3f-ae2c-5e0effb87f2b, 8dea83d1-5beb-4c02-a29f-322205850046, b7864548-2ebe-4823-a884-d1a6455d6a7f, de8f9988-529d-401e-9987-409fab27288a, d3c3086e-4324-4a3f-bf3b-135b8acded07, 685388af-e4d2-4f4f-8b16-029910c08ba6, 64e4a387-ac93-4146-957f-f38c3f6b6e10, 43d8755b-a5f4-4c4d-8aa5-ee3e235b80a1, e314977f-8d03-4eba-9b24-9778b689331b], [11, 1], [10, 0], [12, 2], [11, 1], [13, 3], [9, -1], 5, [], [], [], [d24b8e11-6d39-4228-8211-55f2458fb72c, 52056c6a-5200-4d16-8ecc-cde1ba3e5cd2], [1, 1], [], 250, 'julius.png', '921158705075077150');"""
    )
...
VALUES (f3b9a7c1-2d8e-4f6a-bc21-7e9d5a1d8e54, 'Soco Desarmado', 0, 'acao', 'ARMA, ATAQUE, CONTUNDENTE, LEVE', '3m.', 'Instatânea.', '1d20 + Bônus de FOR.', '1d8 + Bônus de FOR', 'None', 'None', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (b2a9d7f3-8e4d-6c1d-bc71-3e9f7a5d8e54, 'Soco com Manopla', 0, 'acao', 'ARMA, ATAQUE, CONTUNDENTE, PESADO', '5m.', 'Instatânea.', '1d20 + Bônus de FOR.', '1d10 + Bônus de FOR + 2', 'None', 'None', 'Essa habilidade só pode ser usada se o usário tiver usado _"Modificar com Sangue: Conjurar Manoplas"_ antes.', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (7986f0dc-c082-4906-838a-36d991dad853, 'Cólera Ardente', 'Seu sangue quente é capaz de servir como combustível para sua ira controlada. Enquanto estiver ***Machucado***, seus **ATAQUES** recebem a seguinte modificação:', 'reacao', 'ADICIONA', '**Acerto:** Além do normal, você causa +1d6 pontos de dano.\n**Especial:** A partir do 5º nível, você pode aplicar essa modificação quantas vezes quiser.', 1, 'PE');"""
    ...
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (3f9c1b74-8d2e-4a56-bb1c-2e7f5d8a90e3, 'Sangue Maldito', 'Seu sangue impregnado com a essência de Warpinier concede alguns privilégios. O usuário tem **resistência** a dano **ÁCIDO** e **NECRÓTICO**. Além disso, o usuário recebe a habilidade _"Sede de sangue"_.', 'None', 'None', '0', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (59951d9d-3c3e-496b-9491-e7b7554c04ef, 'Transmutar Pacto em Arma', 2, 'acao', 'PACTUADO, MÁGICO', 'Toque.', 'Uma cena.', 'None', 'None', 'None', 'Você canaliza a força de Lumine no objeto. Ele se transforma numa arma que você seja proficiente e você pode usar seu **atributo chave de conjuração**, em vez de **FOR** ou **DES**, para as jogadas de ataque e dano com ela.', 'None', 'None', 'Um objeto mundano que você esteja empunhando.', 'Ilimitado.', 'reacao', 'ADICIONA', '**Efeito:** Você pode dever mais uma obrigação menor para receber vantagem no primeiro ataque que fizer com a arma na cena.', 0, 'PE');"""
    ...
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (45c0cd6c-13e5-48fd-af3e-9bd8e98c564f, 'Sangue Maldito', 'Seu sangue impregnado com a essência de Warpinier concede alguns privilégios. O usuário tem **resistência** a dano **ÁCIDO** e **NECRÓTICO**. Além disso, o usuário recebe a habilidade _"Sede de sangue"_.', 'None', 'None', '0', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (ccbeb133-a5d9-4ad0-a4d9-00a29b2a6998, 'Alimentos das Chamas', 2, 'acao', 'BENÇÃO, MÁGICO', 'Visão.', 'Instântanea.', 'Mágico vs **CAR**.', 'O usuário descobre o que aquela criatura mais deseja (naquela cena).', 'O usuário só pode usar essa habilidade se tiver usado a habilidade "_Receber Benção_"', 'Nenhuma outra personagem ou criatura sabe que você usou essa habilidade.', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.passivas (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (66eed95f-1301-4121-a334-1526177cc8c5, 'Adequar-se ao Meio', 'Após usar a habilidade "_Receber Benção_",o usuário recebe vantagem em todos os testes que envolvam esconder-se ou misturar-se a multidões.', 'None', 'None', '0', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.talentos (id, nome, descricao, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (10d0b1df-e399-4d2f-bd8f-01e0f1ad6b89, 'Corte com Morganas Deathmetal', 0, 'acao', 'ATAQUE, CORTANTE, PESADO, ARMA', '6m.', 'Instântanea.', '1d20 + Bônus de FOR.', '2d6 de pontos de dano.', 'A espada cai', 'None', 'Se o usuário estiver ***Machucado***, ele ganha +1 em testes de ataque.', 'None', 'Uma criatura', '2', 'None', 'None', 'None', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (e6fa95b9-b04c-4f40-88e0-195a0488f0de, 'Foice e corrente', 'Combinação da foice ligada a corrente, para ataque versáteis. "Como essa corrente voltou para você depois que o Tristane desamarrou o Clebim?" -- Disse Chrollo, questinando a ordem dos fatos durante a batalha contra Warpinier.', 25, 3);"""
    )
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (9b8e370b-29f3-470f-bd50-e52f8719f76a, 'Morganas Deathmetal', 'Uma espada feito com os restos da antiga guitarra de Morgana. "No dia que você fizer mais de 6 kills com ela a gente conversa..." -- Disse Chrollo, concretizando a superioriodade da Masked Death', 350, 3);"""
    )
    session.execute(
        """INSERT INTO xmercury.personagens (id, nome, nickname, level, path, classe, legacy, heritage, melancholy, catarse, pe, hp, reducao_de_dano, bonus_de_proficiencia, pericias, talentos, passivas, skills, forca, dexterity, constituicao, inteligencia, sabedoria, carisma, pontos_de_sombra, resistencia, vulnerabilidade, imunidade, inventario_itens, inventario_numero, condicoes, saldo, imagem, usuario)
VALUES (e3f9a5b4-8c6d-4a70-94ff-2b6d2c42e6c8, 'Gunther Nosferata', 'Gunther', 4, 'Pactudo', 'Especialista', 'Sanguir', 'Byotir', 'Você precisa ser convidado para entrar em qualquer propriedade que não lhe pertença, a menos que tenha deixado algum objeto pessoal seu lá dentro, sob o cuidado de outra pessoa do local.', 1, 12, 36, 2, 2, [], [d34c41e1-2f59-4fb9-9429-7dd122e62b19], [45c0cd6c-13e5-48fd-af3e-9bd8e98c564f, 66eed95f-1301-4121-a334-1526177cc8c5], [12cccd1a-79fa-44ac-be51-e2c8ea5c22a3, f53836e1-914b-4ca1-b713-cecd9d518e06, d9f9dab4-913f-416c-9cf6-3690ba853f05, 9b195835-4b3b-4d07-9a80-fa4497b61676, 59951d9d-3c3e-496b-9491-e7b7554c04ef, cd6acb0f-d656-429b-acf2-4987a61d00dc, 88a87ee1-e31f-4985-914e-a040408cf8af, ccbeb133-a5d9-4ad0-a4d9-00a29b2a6998, e7c94b4a-28cb-4a44-956b-1c7ebcf1a0a0, 10d0b1df-e399-4d2f-bd8f-01e0f1ad6b89], [10, 0], [12, 2], [13, 3], [9, -1], [11, 1], [11, 1], 5, ['ÁCIDO', 'NECRÓTICO'], [], [], [e6fa95b9-b04c-4f40-88e0-195a0488f0de, 9b8e370b-29f3-470f-bd50-e52f8719f76a], [1, 1], [], 100, 'gunther.png', '813254664241414144');"""
    )
...
VALUES (bdcaaf43-93af-4979-b1eb-46bd80798415, 'Comandar Companheiro: Buscar e Entregar', 1, 'acao bonus', 'MESTRE DAS FERAS, VOZ', '9m.', 'Instantânea.', 'None', 'None', 'None', 'Seu companheiro animal pega um item com uma criatura voluntária e entrega para outra, desde que ambas estejam no seu alcance. Também é possível usar esse comando para fazer com que ele busque um item arremessado em alcance.', 'None', 'None', 'None', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    ...
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (2fb564b1-61d7-49db-b559-1d2cfa7dc217, 'Comandar Companheiro: Ataque de Superioridade', 0, 'acao bonus', 'MESTRE DAS FERAS, VOZ', '9m.', 'Instantânea.', 'FOR vs FOR', 'Você deve escolher um efeito entre: Deixar o alvo ***Caído***; Pegar um item do alvo; Deixar o alvo ***Agarrado***.', 'None', 'None', 'Você só pode usar essa habilidade em criaturas de seu tamanho ou menor.', 'None', 'Uma criatura.', 'Ilimitado.', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
...
VALUES (671efb4c-0ff7-4f4b-9d43-3b3f53ca6d93, 'Avaliar a Natureza do Inimigo', 1, 'acao bonus', 'ESPECIALISTA', '9m.', 'Instantânea.', '**SAB** vs **SAB**.', 'Você descobre uma informação importante sobre o inimigo. Escolha um: sua **Proteção** mais alta ou sua **Proteção** mais  baixa', 'None', 'None', 'None', 'None', 'Uma criatura.', 'Ilimitado.', 'reacao', 'AMPLIAR +15', '**Execução:** Você pode escolher os dois.', 1, 'PE');"""
    ...
    session.execute(
        """INSERT INTO xmercury.skills (id, nome, custo, execucao, descritores, alcance, duracao, ataque, acerto, erro, efeito, especial, gatilho, alvo, carga, modificador_execucao, modificador_nome, modificador_descricao, modificador_gasto, modificador_gasto_tipo)
VALUES (1a65506a-90ab-4943-ab99-a5aaab829dc7, 'Echo of Pomona: Raio Bola do Serjão dos Foguetes', 4, 'eop', 'EOP, VOZ', '18m.', 'Instantânea.', 'None', '3d10 pontos de dano **ELÉTRICO**.', 'None', 'Vincenzo concentra resquísios da energia de Pomona próximo ao alvo, criando uma esfera de energia que ao acerta deixa o alvo ***Paralisado***', 'None', 'None', 'Criaturas em um diâmetro de 3m.', '1', 'None', 'None', 'None', 0, 'None');"""
    )
    session.execute(
        """
"""
...
"""
    ...
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (7f123601-d48c-4a50-93f2-91fef0b16b60, 'Lâminas duplas', 'Um grande par de láminas encontradas na salão de Warpinier. "Como você ficou tão bom em usar duas espadas do nada?" -- endagou Chrollo.', 35, 2);"""
    )
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (0c94309b-cd29-4945-a539-b420ada16652, 'Soul Guitar', 'Um rifre de pólvora disfaçado de guitarra que antes pertencia a Skeleton.', 100, 3);"""
    ...
    session.execute(
        """INSERT INTO xmercury.itens (id, nome, descricao, preco, volume)
VALUES (cae511af-c8fc-41de-9a7c-4e09fe281e5f, 'Arco Mágico', 'Um arco mágico que você ganhou de Pomona no começo da sua jornada.', 30, 3);"""
    )
    session.execute(
        """
"""
