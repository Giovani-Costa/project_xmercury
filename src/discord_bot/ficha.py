...
class PaginaFicha(discord.ui.View):
    ...
    def criar_ficha(self) -> discord.Embed:
        if self.pagina_atual == 0:  # Ficha
            ...
            embed = discord.Embed(
                title=f"Inventário | {self.personagem.limite_de_volume - self.personagem.volume_atual} espaços livres",
                description=f"",
                colour=discord.Colour.from_str("#226089"),
            )
            ...
            for i in self.personagem.skills:
                ...
                elif i.execucao == "acao bonus":
                    execucao = "<:acao_bonus:1326585197004722197>"
                elif i.execucao == "acao livre":
                    execucao = "<:acao_livre:1326585198892154901>"
                elif i.execucao == "reacao":
                ...
                if i.duracao is not None:
                    ficha_skill += f"\n**Duração:** {i.duracao}"
                if i.gatilho is not None:
                    ficha_skill += f"\n**Gatilho:** {i.gatilho}"
                if i.alvo is not None:
                    ficha_skill += f"\n**Alvo:** {i.alvo}"
                if i.ataque is not None:
                    ficha_skill += f"\n**Ataque:** {i.ataque}"
                if i.acerto is not None:
                    ficha_skill += f"\n**Acerto:** {i.acerto}"
                if i.efeito is not None:
                    ficha_skill += f"\n**Efeito:** {i.efeito}"
                if i.erro is not None:
                    ficha_skill += f"\n**Erro:** {i.erro}"
                if i.carga is not None:
                    ficha_skill += f"\n**Cargas:** {i.carga}"
                for m in i.modificadores:
                    if m.execucao == "acao":
                        m_execucao = "<:acao:1326585196232966225>"
                    elif m.execucao == "acao bonus":
                        m_execucao = "<:acao_bonus:1326585197004722197>"
                    elif m.execucao == "acao livre":
                        m_execucao = "<:acao_livre:1326585198892154901>"
                    elif m.execucao == "reacao":
                        m_execucao = "<:reacao:1326585200519544885>"
                    elif m.execucao == "eop":
                        m_execucao = "<:eop:1327039605790343168>"
                    modificador_skill = f"{m_execucao}  +{m.gasto} {m.gasto_tipo.upper()} [{m.nome}]\n{m.descricao}"
